...
import { searchSoc, type OccSearchResult } from "@/lib/occupation";

type Props = {
  onSelect: (result: OccSearchResult) => void;
};

export function OccupationSearchBar({ onSelect }: Props) {
  const [query, setQuery] = useState("");
  const [open, setOpen] = useState(false);
  const [highlight, setHighlight] = useState(0);
  const wrapRef = useRef<HTMLDivElement>(null);

  const results = useMemo(() => searchSoc(query), [query]);
  ...
  const choose = (r: OccSearchResult) => {
    onSelect(r);
    setOpen(false);
  };
  ...
  return (
    <div ref={wrapRef} className="pointer-events-auto relative w-80">
      <input
        ...
        onKeyDown={onKeyDown}
        placeholder="Search occupations (e.g. nurse, software, truck driver)…"
      ...
      {open && results.length > 0 && (
        <div className="absolute left-0 right-0 top-full mt-1 max-h-96 overflow-y-auto rounded-md border border-zinc-800 bg-zinc-900/95 shadow-lg backdrop-blur-sm">
          {results.map((r, i) => (
            ...
            >
              <span className="font-mono text-zinc-500 shrink-0 w-16">
                {r.node.code}
              </span>
